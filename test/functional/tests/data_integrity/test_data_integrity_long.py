...
runtime_io = datetime.timedelta(days=2)
runtime_timeout = runtime_io + datetime.timedelta(hours=2)
...
def test_data_integrity_long_with_io_classification(filesystems):
    ...
    with TestRun.step("Run test workloads on filesystems with verification"):
        ...
        fio.base_cmd_parameters.set_param('alloc-size', 1024 * 1024)
        ...
        fio_run.verify_dump()
        fio_run.run_time(runtime_io)
        fio_run.read_write(ReadWrite.randrw)
        fio_run.io_depth(128)
        fio_run.blocksize_range([(start_size, stop_size)])
        for core in cores:
            start = 512
            file_section_size = 14 * 1024 * 1024 * 1024  # 14 GiB per IO class
            for _ in range(11):  # 11 IO classes for different sizes of files
                end = 4 * start
                if end < 4 * 1024:
                    end = 4 * 1024
                elif end > 2 * 1024 * 1024 * 1024:
                    end = 2 * 1024 * 1024 * 1024
